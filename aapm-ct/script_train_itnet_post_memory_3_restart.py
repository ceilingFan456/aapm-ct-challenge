...
it_net.load_state_dict(
    torch.load(
        os.path.join(
            config.RESULTS_PATH,
            "ItNet_3_post_mem_id{}_train_phase_1".format(job_id),
...
logging.to_csv(os.path.join(config.RESULTS_PATH, f"ItNet_3_post_mem_id{0}_train_phase_{0}", 'itnet_3_log.csv'), index=False)
