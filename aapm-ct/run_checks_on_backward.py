...
def visualize_results(index, outputs, phantom, difference, results_dir):
    fig, axes = plt.subplots(3, 1, figsize=(5, 15))
    images = [outputs, phantom, difference]
...
with torch.no_grad():
    for i, (phantom, sinogram, fbp) in enumerate(data_loader):
        ...
        outputs = radon_net(sinogram)
        difference = torch.abs(phantom - outputs)

        # Visualize and save combined image
        visualize_results(i, outputs, phantom, difference, results_dir)
