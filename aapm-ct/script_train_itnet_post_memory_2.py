...
for i in range(iter_preinit):
    ...
    it_net_tmp.load_state_dict(
        torch.load(
            os.path.join(
                config.RESULTS_PATH,
                "ItNet_2_mem_id{}_train_phase_1".format(job_id),
...
logging.to_csv(os.path.join(config.RESULTS_PATH, f"ItNet_2_post_mem_id{0}_train_phase_{0}", 'itnet_post_2_log.csv'), index=False)
